...
function App() {
  ...
  async function fetchAllData() {
    ...
    try {
      const res = await fetch(`${apiBaseUrl}/api/correlations`);
      const data = await res.json();
    ...
    try {
      const res = await fetch(`${apiBaseUrl}/api/baselines`);
      const data = await res.json();
    ...
    try {
      const res = await fetch(`${apiBaseUrl}/api/containers`);
      const data = await res.json();
  ...
  const isBenign = (r) => r.classification.includes("Benign") || r.action === "observe";
  const activeProblems = reports.filter((r) => !isBenign(r) && (r.requiresHumanApproval || !r.autoExecute));
  const solvedProblems = reports.filter((r) => r.autoExecute && !r.requiresHumanApproval);
  const benignTelemetry = reports.filter((r) => isBenign(r));
  ...
  return (
    <div className="container">
      ...
      <div className="tabs">
        ...
        >
          Benign Telemetry ({benignTelemetry.length})
        </button>
      </div>
          ...
          <div key={container.id} className="card">
            ...
            <div className="card-body">
              ...
              <div style={{ display: 'flex', gap: '10px', marginTop: '15px' }}>
                <button className="action-btn" onClick={() => fetch(`${apiBaseUrl}/api/containers/quarantine`, { method: 'POST', body: JSON.stringify({container_id: container.id}), headers: {'Content-Type': 'application/json'} })}>Quarantine</button>
                <button className="action-btn" style={{color: 'var(--accent-active)', borderColor: 'var(--accent-active)'}} onClick={() => fetch(`${apiBaseUrl}/api/containers/kill`, { method: 'POST', body: JSON.stringify({container_id: container.id}), headers: {'Content-Type': 'application/json'} })}>Kill</button>
              </div>
            </div>
          </div>
        ))}

        {tab === 'telemetry' && benignTelemetry.length === 0 && (
          <p style={{color: 'var(--text-secondary)'}}>No benign telemetry recorded.</p>
        )}
        {tab === 'telemetry' && benignTelemetry.slice(0, 50).map((report) => (
          <div key={report._id} className="card" onClick={() => viewReportDetails(report._id)}>
            <div className="card-header">
              <h3>{report.incidentId}</h3>
              <span className="badge neutral-badge">Benign</span>
            </div>
            <div className="card-body">
